...
{
    ROS_INFO("Messaggio ricevuto");
    if (msg.data == 1)
    {
        error = 0;
        pre_error = 0;
        control_state = PID;
        ROS_INFO("Cambio stato");
    }
        ...
        */
        ...
        case MAN:
        {
            ...
            if(new_target_width != target_width)
            {
                ...
                control_msg.output = move_goal.width;
                control_msg.output_sat = move_goal.width;      // position of gripper
              //  control_output_pub.publish(control_msg);
            }
            break;
        }
