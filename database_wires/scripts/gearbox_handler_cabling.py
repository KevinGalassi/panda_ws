...
def generatePathKeypoints(resp):
    world_frame = transformations.tfToKDL(listener.lookupTransform('/world', '/gearbox', rospy.Time(0)))
    channels_path = re.sub('=','',resp.wire_path)
    channels=channels_path.split('\'')
    count = 0

    #Set first component intersection point
    channel_pose = world_frame * gearbox.channels.channel_map[channels[0]].getChannelFrame()
    channel_direction = gearbox.channels.channel_map[channels[0]].getChannelDirection()
    intersection_frame = Pose()
    intersection_frame.orientation.x = 0
    intersection_frame.orientation.y = 0
    intersection_frame.orientation.z = 0
    intersection_frame.orientation.w = 1

    if channel_direction[0] == 1:
        intersection_frame.position.x = resp.origin.hole_pose.position.x
        intersection_frame.position.y = channel_pose.p.y()
        intersection_frame.position.z = 0
    else: 
        intersection_frame.position.x = channel_pose.p.x()
        intersection_frame.position.y = resp.origin.hole_pose.position.y
        intersection_frame.position.z = 0
    resp.path_keypoints.append(copy.deepcopy(intersection_frame))

    #Set middle intersection points
    if len(channels)>1:
        count = 0
        while count<len(channels)-1:
            ref_frame = world_frame * gearbox.channels.channel_map[channels[count]].intersection_list[channels[count+1]]
            intersection_frame.position.x = ref_frame.p.x()
            intersection_frame.position.y = ref_frame.p.y()
            resp.path_keypoints.append(copy.deepcopy(intersection_frame))
            count = count+1
    
    #set Last component intersection
    channel_pose = world_frame * gearbox.channels.channel_map[channels[count]].getChannelFrame()
    channel_direction = gearbox.channels.channel_map[channels[count]].getChannelDirection()
    if channel_direction[0] == 1:
        intersection_frame.position.x = resp.destination.hole_pose.position.x
        intersection_frame.position.y = channel_pose.p.y()
        intersection_frame.position.z = 0
    else: 
        intersection_frame.position.x = channel_pose.p.x()
        intersection_frame.position.y = resp.destination.hole_pose.position.y
        intersection_frame.position.z = 0
    resp.path_keypoints.append(copy.deepcopy(intersection_frame))
...
while not rospy.is_shutdown():
    ...
    broadcastTransform(br, correction, "gearbox",root.getName(),
                        time=current_time)
    ...
    if state == State.FULL_TARGET:
        ...
        for item in pin_targets:
            broadcastTransform(br, fromPoseToKdl(item.screw_pose), item.pin + '_' + 'screw', 'world', time=current_time)
            broadcastTransform(br, fromPoseToKdl(item.hole_pose), item.pin + '_' + 'hole', 'world', time=current_time)


    if state == State.SCREW:
        for item in pin_targets:
           broadcastTransform(br, fromPoseToKdl(item.screw_pose), item.pin + '_' + 'corrected_screw', 'world', time=current_time)
           broadcastTransform(br, fromPoseToKdl(item.hole_pose), item.pin + '_' + 'corrected_hole', 'world', time=current_time)
