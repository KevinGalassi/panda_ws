...
int main(int argc, char** argv){
	...
	// Filter
	int filter_size = 5;
	std::vector<std_msgs::Float32MultiArray> filter_window;
	filter_window.resize(filter_size);
	for(int i=0; i< filter_size; i++)
		filter_window[i].data.resize(5);

	std_msgs::Float32MultiArray filter_mean;
	filter_mean.data.resize(5);
	...
	while(ros::ok()){
		...
		/*	
		// Publish on topics
		filter_window[publish_counter].data[0] = first_order_params[0];		// M
		filter_window[publish_counter].data[1] = first_order_params[1];		// n
		filter_window[publish_counter].data[0] = second_order_params[0];	// a
		filter_window[publish_counter].data[1] = second_order_params[1];	// b
		filter_window[publish_counter].data[2] = second_order_params[2];	// c
		...
		if(publish_counter >= 5)
		{
			...
			{
				for(int k=0; k<5; k++)
					filter_mean.data[k] += filter_window[i].data[k];
			}

			for(int i=0; i<5; i++)
				filter_mean.data[i] = filter_mean.data[i]/filter_size;

			first_order_params_msg.data[0]  = filter_mean.data[0];		// M
			first_order_params_msg.data[1]  = filter_mean.data[1];		// n
			second_order_params_msg.data[0] = filter_mean.data[2];		// a
			second_order_params_msg.data[1] = filter_mean.data[3];		// b
			second_order_params_msg.data[2] = filter_mean.data[4];		// c
			
			for(int i=0; i<filter_size; i++)
				filter_mean.data[i] = 0;
