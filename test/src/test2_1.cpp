...
int main(int argc, char** argv)
{
    ...
    */
    float velocity_fix = 0.32;
    float velocity_round = 0.005;
    float velocity_pass = 0.55;
...
/***************  SYSTEM PARAMETERS **************************/
    ...
    param.circ_point = 500;             // Point generated in the semi-circle
    ...
    param.corner_points = 300;          // Numver of point generated in the corner.
...
/**************** COMPUTE TRAJECTORY PARTS      ******************/
    ...
    visual_tools.deleteAllMarkers();
    visual_tools.prompt("FOr thesis, Next for input points");
    for(int i=0; i< waypoints.point.poses.size(); i++)
        visual_tools.publishAxisLabeled(waypoints.point.poses[i], waypoints.pt_label[i], rvt::SMALL);
    visual_tools.trigger();


    MyTrajectoryPlanner3(param, waypoints);

    visual_tools.prompt("NExt for the Secondary Trajectory");
    visual_tools.deleteAllMarkers();

    for(int i=0; i< waypoints.SecondaryTrajectory.size(); i++)
    {
        if(waypoints.pt_label[i] == "fix" || waypoints.pt_label[i] == "cornerfix")
            visual_tools.publishPath(waypoints.SecondaryTrajectory[i].poses, rvt::BLUE, rvt::SMALL);
    }
    for(int i=0; i< waypoints.point.poses.size(); i++)
        visual_tools.publishAxisLabeled(waypoints.point.poses[i], waypoints.pt_label[i], rvt::SMALL);
    visual_tools.trigger();
    ...
    visual_tools.prompt("Next to add the corner");
    visual_tools.deleteAllMarkers();
    ...
    for(int i=0; i< waypoints.SecondaryTrajectory.size(); i++)
    {
        ...
        if(waypoints.pt_label[i] == "corner")
            visual_tools.publishPath(waypoints.SecondaryTrajectory[i].poses, rvt::RED, rvt::SMALL);
    }
    visual_tools.trigger();
    ...
    for(int i=0; i< waypoints.SecondaryTrajectory.size(); i++)
    {
        ...
        if(waypoints.pt_label[i] == "corner")
            ...
            visual_tools.publishPath(waypoints.SecondaryTrajectory[i].poses, rvt::GREEN, rvt::SMALL);
    }
    visual_tools.trigger();

    

    visual_tools.prompt("FInal");
    for(int i=0; i<waypoints.SecondaryTrajectory.size(); i++)
    {
    ...
    */
    ...
    for(int i=0; i<PlansVector.size(); i++)
    {
        if(waypoints.pt_label[i] == "pass")
        {
            ...
            ros::Duration(0.05).sleep();        // aL PSOTO DI 1 -> 0.05
        }
        else
        {
            cmd_msg.data = 2;
