...
bool FrankaVelocityController::init(hardware_interface::RobotHW* robot_hardware,
  ...
  vel_cmd_sub = controller_node_handle.subscribe("/cartesian_velocity_request", 10, &FrankaVelocityController::Velocity_callback, this);
...
void FrankaVelocityController::starting(const ros::Time& time ) {
  ROS_INFO("STARTING COMPLETED");
}


void FrankaVelocityController::update(const ros::Time& time, const ros::Duration& /* period */) {
...
void FrankaVelocityController::stopping(const ros::Time& /*time*/) {
  // WARNING: DO NOT SEND ZERO VELOCITIES HERE AS IN CASE OF ABORTING DURING MOTION
  // A JUMP TO ZERO WILL BE COMMANDED PUTTING HIGH LOADS ON THE ROBOT. LET THE DEFAULT
  // BUILT-IN STOPPING BEHAVIOR SLOW DOWN THE ROBOT.
}

void FrankaVelocityController::Velocity_callback(const std_msgs::Float32MultiArray& msg)
{
  int k=0;
  ...
  if(k == 6)
  {
    ...
    for(int i=0; i<7; i++)
      velocity_joint_handles_[i].setCommand(0.0000000000001);
  }
  else
  {
    for(int i=0; i<6;i++)
...
PLUGINLIB_EXPORT_CLASS(franka_robot_controllers::FrankaVelocityController, 
